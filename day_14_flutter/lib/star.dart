...
class Star extends StatelessWidget {
  ...
  const Star({required this.color, required this.size});
  @override
...
class StarRanking extends StatelessWidget {
  ...
  const StarRanking({super.key, this.color = Colors.deepPurpleAccent, required this.value, this.starSize = 25});

  @override
  Widget build(BuildContext context) {
    return Row(
      children: List.generate(value, (_) => Padding(padding: const EdgeInsets.all(2.0),
        child: Star(
          color: color,
          size: starSize,
        ),
