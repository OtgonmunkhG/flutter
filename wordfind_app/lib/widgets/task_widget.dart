...
class TaskWidgetState extends State<TaskWidget> {
  ...
  Widget build(BuildContext context) {
    TaskModel currentQues = listQuestion[indexQues];
    return SizedBox(
  ...
  void generatePuzzle({
    ...
    if (loop != null) {
      indexQues = 0;
      listQuestion = [];
      listQuestion.addAll(loop);
    } else {
      if (next && indexQues < listQuestion.length - 1) {
        indexQues++;
      } else if (left && indexQues > 0) {
        indexQues--;
      } else if (indexQues >= listQuestion.length - 1) {
        return;
      }

      setState(() {});

      if (listQuestion[indexQues].isDone) return;
    }
